...
def _patch_chat_openai(monkeypatch, classify_result, recommend_result):
    """Patch ChatOpenAI and return the two fake structured models, so tests can
...
def test_classify_prompt_contains_every_event_message(monkeypatch):
    ...
    fakes = _patch_chat_openai(monkeypatch, classify_result=canned, recommend_result=None)
    ...
    OpenAIClient(api_key="fake-key").classify(events)
...
def test_recommend_prompt_contains_incident_summary_category_and_severity(monkeypatch):
    """Pins the CURRENT contents of _incident_to_prompt (shared with the Anthropic adapter).
    ...
    """
    remediation = Remediation(incident_id="inc-001", fix_steps=["Restart pod"], rationale="r", risk="low", effort="low", source="llm")
    fakes = _patch_chat_openai(monkeypatch, classify_result=None, recommend_result=remediation)

    incident = Incident(
        id="inc-042", category="db_timeout", severity=Severity.HIGH, summary="Postgres connections timing out", source_events=[]
    )
    OpenAIClient(api_key="fake-key").recommend(incident, {"fix_steps": ["Raise pool size"], "risk": "low"})
...
def test_recommend_prompt_omits_runbook_section_when_no_entry(monkeypatch):
    ...
    OpenAIClient(api_key="fake-key").recommend(incident, None)
